...
if ( ( $serial == '' ) || ( !is_array( $backup ) ) ) {
	...
	if ( '' != $backup['deployment_log'] ) {
		...
		if ( 'push' == $backup['deployment_direction'] ) {
			...
			pb_backupbuddy::status( 'details', 'About to retrieve pull deployment status log with serial `' . $backup['deployment_log'] . '`...', $serial );
			pb_backupbuddy::status( 'details', '* *** Begin Remote Backup Log section', $serial );
		} else {
