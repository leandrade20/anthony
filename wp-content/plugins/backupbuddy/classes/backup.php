...
class pb_backupbuddy_backup {
	...
	public function deploy_pull_files( $state, $pullBackupArchive = '' ) {
		...
		if ( true !== $state['sendMedia'] ) {
			...
			if ( $mediaFileCount > 0 ) { // Media files remain to send.
				$getFile = array_pop( $state['pushMediaFiles'] ); // Pop off last item in array. Faster than shift.
		...
		if ( true !== $state['sendPlugins'] ) {
			...
			if ( $pluginFileCount > 0 ) { // Plugin files remain to send.
				...
				$getFile = array_pop( $state['pushPluginFiles'] ); // Pop off last item in array. Faster than shift.
		...
		if ( true !== $state['sendTheme'] ) {
			...
			if ( $themeFileCount > 0 ) { // Plugin files remain to send.
				$getFile = array_pop( $state['pushThemeFiles'] ); // Pop off last item in array. Faster than shift.
	...
	public function deploy_getFile( $state, $destinationPath, $sendFile, $sendType, $finalNextStep, $seekTo = 0 ) {
		...
		while( true == $keepSending ) {
			...
			if ( false === ( $fs = fopen( $saveFile, 'a' ) )) {
				$message = 'Error #43834984: Unable to fopen file `' . $saveFile . '`.';
				pb_backupbuddy::status( 'error', $message );
				return false;
			}
			...
			if ( '1' == $response['filedone'] ) { // Transfer finished.
				...
				pb_backupbuddy::status( 'details', 'Retrieval of chunk + writing took `' . $elapsed . '` seconds. Got up to `' . pb_backupbuddy::$format->file_size( $maxPayload ) . '` MB of `' . pb_backupbuddy::$format->file_size( $size ) . '` MB. Encoded size received: `' . $size . '` bytes.');
				...
				if ( ( ( $elapsed + $TIME_WIGGLE_ROOM ) * ( $numSentThisRound + 1 ) ) >= $state['minimumExecutionTime'] ) { // Could we have time to send another piece based on average send time?
					...
					$nextStep = array(
						...
						'args'			=>	array( $state, $destinationPath, $sendFile, $sendPath, $sendType, $finalNextStep, $seekTo ),
						'start_time'	=>	0,
						'finish_time'	=>	0,
						'attempts'		=>	0,
					);
	...
	public function deploy_pull_renderImportBuddy( $state ) {
		...
		$password = $state['destination']['key_public'];
		$siteurl = site_url();
		
		
		$importFileSerial = backupbuddy_core::deploymentImportBuddy( $password, basename( $state['pullLocalArchiveFile'] ) );
		...
		pb_backupbuddy::status( 'details', 'Load importbuddy at `' . $importbuddyURLRoot . '` with verifier `' . $password . '`.' );
	...
	public function deploy_push_sendContent( $state ) {
		...
		if ( true !== $state['sendMedia'] ) {
			...
			if ( $mediaFileCount > 0 ) { // Media files remain to send.
				...
				$wp_upload_dir = wp_upload_dir();
				$nextStep['args'] = array( $state );
				return $this->deploy_sendFile( $state, $wp_upload_dir['basedir'] . '/' . $sendFile, $sendFile, 'media', $nextStep );
			}
		}
		...
		if ( true !== $state['sendPlugins'] ) {
			...
			if ( $pluginFileCount > 0 ) { // Plugin files remain to send.
				...
				$pluginPath = wp_normalize_path( WP_PLUGIN_DIR ) . '/';
				$nextStep['args'] = array( $state );
				return $this->deploy_sendFile( $state, $pluginPath . $sendFile, $sendFile, 'plugin', $nextStep );
			}
		}
		...
		if ( true !== $state['sendTheme'] ) {
			...
			if ( $themeFileCount > 0 ) { // Plugin files remain to send.
				...
				$themePath = get_template_directory(); // contains trailing slash.
				$nextStep['args'] = array( $state );
				return $this->deploy_sendFile( $state, $themePath . $sendFile, $sendFile, 'theme', $nextStep );
			}
		}
