...
class backupbuddy_core {
	...
	public static function loopback_test() {
		...
		if( is_wp_error( $response ) ) { // Loopback failed. Some kind of error.
			...
			pb_backupbuddy::status( 'error', 'Loopback test error: `' . $error . '`. URL: `' . $loopback_url . '`. If you need to contact your web host, tell tham that when PHP tries to connect back to the site at the URL `' . $loopback_url . '` via cron (or other fallback connection method build into WordPress) that it gets the error `' . $error . '`. This means that WordPress\' built-in simulated cron system cannot function properly, breaking some WordPress features & subsequently some plugins. There may be a problem with the server configuration preventing connections from working properly.' );
			return 'Error: ' . $error;
		} else {
	 ...
	 *
	 * @param	string			$backupFile		Backup filename. Eg.  backup-xijxlxjnx.zip. Will be looked for in the backup directory.
	 ...
	 *
	 */
	public static function deploymentImportBuddy( $password, $backupFile ) {
		
		//$tempDir = backupbuddy_core::getTempDirectory();
		$tempDir = backupbuddy_core::getBackupDirectory();
		$tempFile = $tempDir . str_replace( array( '\\', '/' ), '', $backupFile );
		if ( ! file_exists( $tempFile ) ) {
			$error = 'Error #43848378: Backup file `' . $tempFile . '` not found uploaded.';
			pb_backupbuddy::status( 'error', $error);
		...
		backupbuddy_core::importbuddy( ABSPATH . $importFilename, $password = md5( md5( $password ) ) );
		
		// Render default config file overrides. Overrrides default restore.php state data.
		$state = array();
		global $wpdb;
		$state['type'] = 'deploy';
		$state['archive'] = $tempDir . $backupFile;
